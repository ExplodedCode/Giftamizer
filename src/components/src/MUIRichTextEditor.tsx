...
import Toolbar, { TToolbarControl, TCustomControl, TToolbarButtonSize } from './MUIRichTextEditor/Toolbar';
import Link from './MUIRichTextEditor/Link';
import Media from './MUIRichTextEditor/Media';
import Blockquote from './MUIRichTextEditor/Blockquote';
import CodeBlock from './MUIRichTextEditor/CodeBlock';
import UrlPopover, { TAlignment, TUrlData, TMediaType } from './MUIRichTextEditor/UrlPopover';
import Autocomplete, { TAutocompleteItem } from './MUIRichTextEditor/Autocomplete';
...
// Refactored: was useEditorState, now a plain function
const createEditorState = (props: IMUIRichTextEditorProps) => {
...
const MUIRichTextEditor: ForwardRefRenderFunction<TMUIRichTextEditorRef, IMUIRichTextEditorProps> = (props, ref) => {
	...
	const insertAutocompleteSuggestionAsText = (selection: SelectionState, value: string) => {
		...
		const contentState = Modifier.replaceText(editorStateRef.current!.getCurrentContent(), selection, value, editorStateRef.current!.getCurrentInlineStyle(), entityKey);
		...
		if (autocompleteRef.current!.insertSpaceAfter === false) {
			...
			const addSpaceState = Modifier.insertText(newEditorState.getCurrentContent(), newEditorState.getSelection(), ' ', newEditorState.getCurrentInlineStyle());
	...
	const insertAsyncAtomicBlockPlaceholder = (name: string, placeholder?: string): SelectionState => {
		...
		const contentState = Modifier.insertText(editorStateRef.current!.getCurrentContent(), currentContentState.getSelectionAfter(), placeholderName, undefined, entityKey);
	...
	const handlePastedFiles = (files: Blob[], editorState: EditorState): DraftHandleValue => {
			...
			if (file.type.startsWith('image/')) {
				...
				reader.onloadend = async () => {
					...
					try {
						...
						// Use a default name for clipboard images
						formData.append('name', (file as any).name || 'clipboard.png');
						const response = await (await import('axios')).default.post('https://api.imgbb.com/1/upload', formData);
						const imageUrl = response.data?.data?.url;
						// Insert image block with default width 500
						const data = { url: imageUrl, width: 500, type: 'image' };
						const newEditorState = insertAtomicBlock(editorStateRef.current!, 'IMAGE', data);
	...
	const handlePastedText = (text: string, html: string | undefined, editorState: EditorState): DraftHandleValue => {
		// Try to handle image paste from clipboard (e.g. from Word, screenshots, etc)
		if (html) {
			...
			if (imgMatch && imgMatch[1].startsWith('data:image/')) {
				...
				fetch(imgMatch[1])
					...
					.then((blob) => {
						...
						handlePastedFiles([file], editorState);
					});
				return 'handled';
			}
		}
	...
	return (
		<div id={`${editorId}-root`} className={classes.root}>
			...
			>
					...
					>
						<Editor
							...
							handleDroppedFiles={(selection, files) => {
								// Use the same logic as handlePastedFiles
								return handlePastedFiles(files, editorStateRef.current!);
							}}
				...
				{state.anchorUrlPopover ? <UrlPopover data={state.urlData} anchor={state.anchorUrlPopover} onConfirm={handleConfirmPrompt} isMedia={state.urlIsMedia} /> : null}
			</div>
		</div>
	);
};
